...
char display_write_char(char ch, display_font_t font, display_color_t color) 
{
    ...
    if (DISPLAY_WIDTH < (display.current_x + char_width) ||
        DISPLAY_HEIGHT < (display.current_y + font.height))
...
char display_write_string(char* str, display_Font_t Font, display_COLOR color) {
    while (*str) {
...
void display_draw_line(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, display_COLOR color) {
...
/* Draw polyline */
void display_draw_polyline(const display_VERTEX *par_vertex, uint16_t par_size, display_COLOR color) {
    uint16_t i;
    if(par_vertex == NULL) {
        return;
    }
...
static float display_DegToRad(float par_deg) {
    return par_deg * (3.14f / 180.0f);
}

/* Normalize degree to [0;360] */
static uint16_t display_NormalizeTo0_360(uint16_t par_deg) {
    uint16_t loc_angle;
    if(par_deg <= 360) {
        loc_angle = par_deg;
    } else {
 ...
 * sweep in degree
 */
void display_draw_arc(uint8_t x, uint8_t y, uint8_t radius, uint16_t start_angle, uint16_t sweep, display_COLOR color) {
    ...
    loc_sweep = display_NormalizeTo0_360(sweep);
    
    count = (display_NormalizeTo0_360(start_angle) * CIRCLE_APPROXIMATION_SEGMENTS) / 360;
    approx_segments = (loc_sweep * CIRCLE_APPROXIMATION_SEGMENTS) / 360;
    approx_degree = loc_sweep / (float)approx_segments;
    while(count < approx_segments)
    {
        rad = display_DegToRad(count*approx_degree);
        ...
        if(count != approx_segments) {
            ...
            rad = display_DegToRad(loc_sweep);
        }
 ...
 */
void display_draw_arc_with_radius(uint8_t x, uint8_t y, uint8_t radius, uint16_t start_angle, uint16_t sweep, display_COLOR color) {
    ...
    // Radius line
    display_Line(x,y,first_point_x,first_point_y,color);
    display_Line(x,y,xp2,yp2,color);
    return;
}

/* Draw circle by Bresenhem's algorithm */
void display_DrawCircle(uint8_t par_x,uint8_t par_y,uint8_t par_r,display_COLOR par_color) {
    int32_t x = -par_r;
    int32_t y = 0;
...
void display_fill_circle(uint8_t par_x,uint8_t par_y,uint8_t par_r,display_COLOR par_color) {
    int32_t x = -par_r;
    int32_t y = 0;
    int32_t err = 2 - 2 * par_r;
    int32_t e2;
...
/* Draw a rectangle */
void display_draw_rectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, display_COLOR color) {
...
void display_fill_rectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, display_COLOR color) {
...
display_Error_t display_invert_rectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2) {
  ...
  if ((y1 / 8) != (y2 / 8)) {
    ...
    for (i = x1 + (y1 / 8) * display_WIDTH;
         i <= (uint32_t)x2 + (y2 / 8) * display_WIDTH; i++) {
